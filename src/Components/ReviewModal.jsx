...
function ReviewModal({ project, onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
      <div className="bg-[#0f0e1a] border border-gray-700 rounded-xl p-6 w-full max-w-[440px]">
        <div className="flex justify-between items-center mb-4">
          <p className="text-[16px] font-medium text-gray-50">
            Review Do — {project.title}
          </p>
          <TbX className="text-gray-400 cursor-pointer text-lg" onClick={onClose} />
        </div>

        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          {/* Star Rating */}
          <div>
            <label className="text-[13px] text-gray-400 mb-2 block">Freelancer ko rate karo</label>
          ...
          {/* Comment */}
          <div>
            <label className="text-[13px] text-gray-400 mb-1.5 block">Comment</label>
            <textarea
              value={comment}
