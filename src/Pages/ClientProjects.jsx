...
function ClientProjects() {
  ...
  const statusBadge = (status) => {
    const styles = {
      open: "text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 border-amber-200 dark:border-amber-500/20",
      "in-progress": "text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-500/10 border-green-200 dark:border-green-500/20",
      completed: "text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-500/10 border-blue-200 dark:border-blue-500/20",
    };
  ...
  return (
    <div className="bg-gray-50 dark:bg-[#0a0a0f] min-h-screen pb-8 text-gray-800 dark:text-gray-200 transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-10">
        ...
        <p className="text-[18px] font-medium text-gray-900 dark:text-gray-50 mb-1">My Projects</p>
            ...
            <button
              ...
              className={`text-[12px] font-medium px-4 py-1.5 rounded-full border transition-colors ${
                filter === f
                  ...
                  : "bg-transparent border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200"
              }`}
            >
              {f === "all" ? "Sab" : f}
            </button>
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-3">
            {filteredProjects.map((project) => (
              <div key={project._id} className="bg-white dark:bg-[#ffffff08] border border-gray-200 dark:border-gray-700 rounded-lg p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm dark:shadow-none">
                <div>
                ...
                <div className="flex items-center gap-3">
                  <p className="text-[14px] font-medium text-[#534AB7] dark:text-[#a5b4fc]">
                    ...
                    <>
                      ...
                      {project.status === "in-progress" && (
                        <button
                          ...
                          className="bg-green-50 dark:bg-green-600/20 border border-green-500 dark:border-green-600 text-green-600 dark:text-green-400 text-[12px] font-medium px-3.5 py-1.5 rounded-md hover:bg-green-100 dark:hover:bg-green-600/30"
                        >
                          Mark Completed
                        </button>
                      )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
          <div className="bg-white dark:bg-[#0f0e1a] border border-gray-200 dark:border-gray-700 rounded-xl p-6 w-full max-w-[480px]">
            <div className="flex justify-between items-center mb-4">
              <p className="text-[16px] font-medium text-gray-900 dark:text-gray-50">{taskModal.title} — Tasks</p>
              <TbX className="text-gray-500 dark:text-gray-400 cursor-pointer text-lg" onClick={() => setTaskModal(null)} />
            </div>
            ...
            <form onSubmit={addTask} className="flex gap-2 mb-4">
              <input
                ...
                className="flex-1 bg-white dark:bg-transparent border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 outline-none focus:border-[#534AB7] text-sm"
              />
            ...
            <div className="flex flex-col gap-2 max-h-[300px] overflow-y-auto">
              {tasks.length === 0 ? (
                ...
                tasks.map((task) => (
                  <div key={task._id} className="bg-gray-50 dark:bg-[#ffffff08] border border-gray-200 dark:border-gray-700 rounded-md p-3 flex justify-between items-center">
                    <p className="text-[13px] text-gray-800 dark:text-gray-200">{task.title}</p>
